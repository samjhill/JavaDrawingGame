...
public class VoteServer implements EntryInformation {
	...
	public ArrayList<Entry> top() {
		...
		for (int i=0; i < entries.length; i++) {
			...
			for (int j=0; j < topEntries.size()+1; j++) {
				if (shouldKeepGoing) {
					if (topEntries.size() == 0) { // First entry
						topEntries.add(comparedEntry);
						shouldKeepGoing = false;
					} else if (j == topEntries.size()) { // Last entry
						topEntries.add(comparedEntry);
						shouldKeepGoing = false;
					} else if (topEntries.get(j).getVotes() < comparedEntry.getVotes()) {
			...
			if (topEntries.size() > 7) {
				topEntries.remove(topEntries.size()-1);
			}
		} // end master loop
		return topEntries;
	} // end top();
